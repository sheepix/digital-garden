  ...
  fetch('/tags.json')
    ...
    .then(tagsData => {
      ...
      for (let i = 0; i < tagsArray.length; i++) {
        ...
        html += '<div id="tag-' + encodeURIComponent(item.tag) + '" class="scroll-mt-24 border border-stone-200 dark:border-stone-700 rounded-lg p-6 hover:border-primary/50 transition-colors">';
        html += '<div class="flex items-center justify-between mb-4">';
        html += '<h2 class="text-xl font-semibold text-stone-900 dark:text-white">';
        html += '<a href="#tag-' + encodeURIComponent(item.tag) + '" class="hover:text-primary transition-colors focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none dark:focus-visible:ring-offset-stone-900 rounded">#' + item.tag + '</a>';
        html += '</h2>';
