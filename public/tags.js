  ...
  fetch('/tags.json')
    ...
    .then(tagsData => {
      ...
      for (let i = 0; i < tagsArray.length; i++) {
        ...
        const encodedTag = encodeURIComponent(item.tag).toLowerCase().replace(/%20/g, '-');
        ...
        html += '<a href="#tag-' + encodedTag + '" aria-label="Tag: ' + item.tag + '" class="hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 dark:focus-visible:ring-offset-stone-900 rounded transition-colors">#' + item.tag + '</a>';
        html += '</h2>';
        ...
        for (let j = 0; j < Math.min(item.pages.length, 5); j++) {
          ...
          html += '<a href="/' + page.slug + '" class="block text-stone-600 dark:text-stone-400 hover:text-primary dark:hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 dark:focus-visible:ring-offset-stone-900 rounded transition-colors text-sm">• ' + page.title + '</a>';
        }
