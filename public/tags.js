  ...
  fetch('/tags.json')
    ...
    .then(tagsData => {
      ...
      for (let i = 0; i < tagsArray.length; i++) {
        ...
        const trimmedTag = item.tag.trim();
        if (!trimmedTag) continue;
        const slug = encodeURIComponent(trimmedTag).toLowerCase().replace(/%20/g, '-');
        ...
        html += '<a href="#tag-' + slug + '" class="hover:text-primary transition-colors focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none dark:focus-visible:ring-offset-stone-900 focus-visible:ring-offset-2 rounded-sm">#' + trimmedTag + '</a>';
        html += '</h2>';
