  ...
  fetch('/tags.json')
    ...
    .then(tagsData => {
      ...
      for (let i = 0; i < tagsArray.length; i++) {
        ...
        html += '<div id="tag-' + encodeURIComponent(item.tag) + '" class="border border-stone-200 dark:border-stone-700 rounded-lg p-6 hover:border-primary/50 transition-colors scroll-mt-24">';
        html += '<div class="flex items-center justify-between mb-4">';
        html += '<h2 class="text-xl font-semibold text-stone-900 dark:text-white">';
        html += '<a href="#tag-' + encodeURIComponent(item.tag) + '" class="hover:text-primary transition-colors">#' + item.tag + '</a>';
        html += '</h2>';
