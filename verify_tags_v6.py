...
def test_tags_deep_link(page: Page):
  ...
  # Debug: Print the HTML content of the tag cloud container
  content = page.inner_html('#tag-cloud-content')
  print(f'Tag Cloud Content: {content}')
  ...
  if tag_link.count() > 0:
      ...
      if '#' in tag_href:
          ...
          page.wait_for_url(f'**/tags/#*', timeout=5000)

          current_url = page.url
          print(f'Current URL: {current_url}')

          # Wait for the tags page to load content
          time.sleep(2)

          # 4. Assert: Confirm the target element exists and has the correct ID.
          # We are looking for an element with id='tag-{tagName}'
          target_id = f'tag-{tag_fragment.replace("tag-", "")}' # Ensure prefix is correct
