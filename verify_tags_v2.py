...
def test_tags_deep_link(page: Page):
  ...
  if tag_link.count() > 0:
      ...
      if '#' in tag_href:
          ...
          # We check if the element is attached to the DOM, as visibility might be tricky with scrolling
          if target_element.count() > 0:
              print(f'Target element found: #{tag_id}')
              ...
              print(f'Target element NOT found: #{tag_id}')
